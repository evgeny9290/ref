...
class COPLocalSearchAlgorithmsAveraging:
    ...
    def create_final_run_and_run_algs_from_final_run(self):  # both create final_run file if doesnt exist and run prog with those params
    ...
    def _run_with_best_params_and_create_expected_graphs(self, print_graphs_bool):  # private func for find_best_params_run_them_output_expected_graphs
        self.create_final_run_and_run_algs_from_final_run()
    ...
    def find_best_params_run_them_output_expected_graphs(self, print_graphs_bool):  # both create final_run + run with best params + create graphs
        self.remove_all_files_in_path(self.results_path)  # clear results folder
        self._run_with_best_params_and_create_expected_graphs(print_graphs_bool)  # run prog with best params and create expected graphs
...
if __name__ == '__main__':
    ...
    num_iterations = 20  # used for optuna as number of trials, has to be accurate
    ...
    COP_automized_run.find_best_params_run_them_output_expected_graphs(print_graphs_bool=True)  # if optimal params already exist run this
    # COP_automized_run.create_expected_graphs(print_graphs_bool=True)